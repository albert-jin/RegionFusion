...
class VOC12ClsDataset(VOC12Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Strict image-label-only training: do not load or inspect pixel GT.
        img_name = self.name_list[idx]
        image = np.asarray(imageio.imread(os.path.join(self.img_dir, img_name + '.jpg')))
        image = transforms.random_scaling(image, scale_range=self.rescale_range, label=None)
        image = transforms.random_fliplr(image, label=None)
        image, img_box = transforms.random_crop(image, label=None, crop_size=self.crop_size,
                                               mean_rgb=[0, 0, 0], ignore_index=self.ignore_index)
        image = np.transpose(transforms.normalize_img(image), (2, 0, 1))
        cls_label = self.label_list[img_name]
        # The training loop keeps its tuple interface; this mask is unused by losses.
        label = np.full((self.crop_size, self.crop_size), self.ignore_index, dtype=np.uint8)
