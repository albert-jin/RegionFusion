...
### dataset utils
parser.add_argument("--data_folder", required=True, type=str, help="dataset folder")
...
parser.add_argument("--local_rank", "--local-rank", default=-1, type=int, help="local_rank")
parser.add_argument("--num_workers", default=10, type=int, help="num_workers")
parser.add_argument('--teacher_dir',default=None,help='Complete offline model-generated training pseudo labels')
...
def train(args=None):
    ...
    ### build dataloader
    dataset_cls=voc.VOC12ClsDataset
    teacher_args={}
    if args.teacher_dir:
        from datasets.teacher_voc import VOC12TeacherDataset
        dataset_cls=VOC12TeacherDataset;teacher_args={'teacher_dir':args.teacher_dir}
        assert args.num_workers>0, 'GT audit belongs to isolated training workers'
        logging.info('OFFLINE_TEACHER=%s; strict image-only crop sampling; original diversity schedule',args.teacher_dir)
    ...
    for n_iter in range(args.max_iters):
        ...
        seg_targets=labels if args.teacher_dir else aff_pseudos
        seg_loss = get_seg_loss(segs, seg_targets.type(torch.long), ignore_index=args.ignore_index)
        ...
        _, diver_pseudo_labels = lam_to_label(attr_maps_raw.permute(0,2,1).reshape(inputs.shape[0], args.num_classes-1, inputs.shape[-2]//16, inputs.shape[-1]//16), cls_labels.cuda(), img_box=None, ignore_mid=False, bkg_thre=args.bkg_thre, high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index)
    ...
    logging.info('CUDA_PEAK_ALLOCATED_MIB=%.3f CUDA_PEAK_RESERVED_MIB=%.3f', torch.cuda.max_memory_allocated()/2**20, torch.cuda.max_memory_reserved()/2**20)
    return True
