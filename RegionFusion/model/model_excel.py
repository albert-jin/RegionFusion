...
class ExCEL_model(nn.Module):
    def __init__(self,  clip_model=None, embedding_dim=256, in_channels=512, dataset_name='pascal_voc', \
                        num_classes=21, num_atrr_clusters=112, json_file='./attributes_text/descriptors_pascal_voc_gpt4.0_cluster_a_photo_of4.json',\
        ...
        # Preserve the original initialization RNG stream for the data loader.
        from .radio_dense import FrozenRadioSpatial
        device_index=torch.device(device).index
        if device_index is None:device_index=torch.cuda.current_device()
        with torch.random.fork_rng(devices=[device_index]):
            self.radio_visual=FrozenRadioSpatial(device)
            self.radio_projection=nn.Conv2d(1152,self.embedding_dim,kernel_size=1)
            nn.init.zeros_(self.radio_projection.weight)
            nn.init.zeros_(self.radio_projection.bias)
        self.radio_fusion_enabled=True
    ...
    def get_param_groups(self):
        ...
        for param in list(self.decoder_fts_fuse.parameters()):
            param_groups[3].append(param)
        param_groups[3].extend(self.radio_projection.parameters())
    ...
    def forward(self, img, ex_feats=None):
        ...
        seg_fts=fts
        if self.radio_fusion_enabled:
            radio_fts=self.radio_visual(img)
            seg_fts=fts+self.radio_projection(radio_fts)
        seg, seg_attn_weight_list = self.decoder(seg_fts)
